...
void validate(NeuralNetwork &nn, matrix<> &ti, matrix<> &to) {
  ...
  for (unsigned i = 0; i < ti.getRows(); i++) {
    ...
    std::cout << ti.value(i, 0) << " ^ " << ti.value(i, 1) << " = "
...
int main(int argc, char *argv[]) {
  ...
  if (ti.getRows() != to.getRows()) {
    fprintf(stderr, "\e[31m<GYM> Number of samples in input and output do not "
                    "match!\n\e[0m");
    return 1;
  }
  if (ti.getCols() != arch[0]) {
    fprintf(stderr, "\e[31m<GYM> Number of features in input and architecture "
                    "do not match!\n\e[0m");
    return 1;
  }
  if (to.getCols() != arch.back()) {
    fprintf(stderr, "\e[31m<GYM> Number of features in output and architecture "
                    "do not match!\n\e[0m");
    return 1;
  }
